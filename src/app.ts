...
// Directs all requests starting with /Posts to the postRouter
app.use("/Posts", postRouter);

// This is default route it returns documentation of the API. It uses PUG to generate it.
app.use("/", (req,res,next)=>{
    let apiEndPoints=[];
    apiEndPoints.push({method:'[GET]', url:'/Users', description:'Should return a list of any user in the database (as a JSON array) with all their properties except password'});
    ...
    apiEndPoints.push({method:'[GET]', url:'/Users/:userId', description:'Returns a User object except password for the specified userId, returns 404 Error if the user isn\'t found'});
    apiEndPoints.push({method:'[PATCH]', url:'/Users/:userId', description:'Updates an existing user passed dynamically in the URl. Updatable properties include firstName, lastName, emailAddress, password. Returns 404 error if User is not found'});
